...
export default function VideoAulasConjuntosScreen() {
  ...
  const [currentVideoId, setCurrentVideoId] = useState(playlistConjuntos.videos[0].id);
  ...
  const onFullScreenChange = useCallback ((isFullScreen: boolean) => { //muda orientação da tela
    if(isFullScreen){
  ...
  const onStateChange = useCallback((state: string) => { //monitora estado do player
    if (state === 'ended') {
      setIsPlaying(false);
  ...
  const currentVideoDetails = playlistConjuntos.videos.find(v => v.id === currentVideoId); //infos do video atual

  return (
    <LinearGradient style={styles.container} colors={[Colors.gradientStart, Colors.gradientEnd]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        ...
        <View style={styles.videoInfoContainer}>
          <Text style={styles.playlistTitle}>{currentVideoDetails?.title || playlistConjuntos.title}</Text>
          <Text style={styles.playlistCreator}>Criado por {playlistConjuntos.creator}</Text>
          <View style={styles.durationContainer}>
            <Feather name="clock" size={14} color="#555" />
            <Text style={styles.durationText}>{playlistConjuntos.totalDuration}</Text>
          </View>
        </View>
        
        <View style={styles.playlistButton}>
            <Text style={styles.playlistButtonText}>Playlist ({playlistConjuntos.videos.length})</Text>
        </View>
