...
import { playlistFatSis, Video } from "@/constants/dadosVideos";
...
export default function VideoAulasConjuntosScreen() {
  ...
  const currentVideoDetails = playlistFatSis.videos.find(
    ...
    >
      ...
      <View style={styles.videoInfoContainer}>
        <Text style={styles.playlistTitle}>
          {currentVideoDetails?.title || playlistFatSis.title}
        </Text>
        <Text style={styles.playlistCreator}>
          Criado por {playlistFatSis.creator}
        </Text>
        <View style={styles.durationContainer}>
          <Feather name="clock" size={14} color="#555" />
          <Text style={styles.durationText}>
            {playlistFatSis.totalDuration}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.playlistButton}>
        <Text style={styles.playlistButtonText}>
          Playlist ({playlistFatSis.videos.length})
        </Text>
      </View>
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        <View style={styles.videoListContainer}>
          {playlistFatSis.videos.map((video, index) => (
            <TouchableOpacity
              key={video.id}
